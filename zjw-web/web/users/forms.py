...
class RegisterForm(UserCreationForm):  #注册表单
	username = forms.CharField(
		error_messages={"required":u"请输入你的用户名"},
		widget=forms.TextInput(
			attrs={
					'class':'form-control',
					'placeholder':"请输入你的用户名",
					'required':'required',
					'oninvalid':"setCustomValidity('请输入你的用户名');",
	...
	sex = forms.ChoiceField(
		error_messages={"required":u"请选择你的性别"},
	...
	email = forms.EmailField(
		error_messages={"required":u"请输入你的邮箱地址"},
		widget=forms.EmailInput(
			attrs={
				'class':"form-control",
				'placeholder':"请输入你的电子邮箱地址",
				'required':'required',
				'oninvalid':"setCustomValidity('请输入你的电子邮箱地址');",
				'oninput':"setCustomValidity('');"
			}
		)
	)
	...
	password1 = forms.CharField(
		...
		widget=forms.PasswordInput(
			attrs={
				...
				'placeholder':"请设置你的密码",
				'required':"required",
				'oninvalid':"setCustomValidity('请设置你的密码');",
				'oninput':"setCustomValidity('');"
			}
		)
	)
	...
	password2 = forms.CharField(
		...
		widget=forms.PasswordInput(
			attrs={
				...
				'required':"required",
				'oninvalid':"setCustomValidity('请重新输入你的密码');",
				'oninput':"setCustomValidity('');"
			}
		)
	)
...
class ChangeForm(forms.ModelForm):   #个人信息更改表单
	pic = forms.ImageField(
		widget=forms.FileInput(
			attrs={
				'accept':"image/*",
				'oninvalid':"setCustomValidity('请上传图片');",
				'oninput':"setCustomValidity('');"
			}
		),
		required=False
...
class LoginForm(AuthenticationForm):  #登录表单
	captcha = CaptchaField(
		error_messages={"invalid":u"验证码错误","required":u"请输入验证码"},
		required=True
	)
